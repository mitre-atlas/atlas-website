  ...
  <div v-if="dataObject != undefined">
    ...
    <v-row>
      <v-col :cols="mdAndUp ? 9 : 12">
        ...
        />
        <v-list-item>
          <div v-html="markdown.render(dataObject.description)" />
        </v-list-item>
      </v-col>

      <v-col :cols="mdAndUp ? 3 : 12">
        <DataSidebar :data-object="dataObject" variant="outlined" />
      </v-col>
    </v-row>
...
const hasSubtechniques = computed(() => {
  return relatedObjects.value.subtechniques?.length > 0
})

const hasOtherSubtechniques = computed(() => {
  return relatedObjects.value['other subtechniques']?.length > 0
})
...
const filteredRelatedObjects = computed(() => {
      ...
      cs.procedure.forEach((p) => {
        if (p.technique == dataObject.value.id) {
          procedure_examples.push({
            ...
            tactic: {
              name: relatedObjects.value.tactic
                ? relatedObjects.value.tactic.filter((tactic) => tactic.id == p.tactic)[0].name
                : relatedObjects.value.tactics.filter((tactic) => tactic.id == p.tactic)[0].name,
              route: relatedObjects.value.tactic
                ? relatedObjects.value.tactic.filter((tactic) => tactic.id == p.tactic)[0].route
                : relatedObjects.value.tactics.filter((tactic) => tactic.id == p.tactic)[0].route
            },
  ...
  return Object.fromEntries(
    Object.entries(relatedObjects.value).filter(([key]) => !ignore.includes(key))
  )
})
