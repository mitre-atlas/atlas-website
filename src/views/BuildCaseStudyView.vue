<template>
  ...
  <v-row>
    <UploadFileDialog @submit="fillForm" />
    <v-spacer />
    <InstructionsDialog />
  </v-row>
  <p class="text-h6 mb-0 mt-6">Details</p>
  <p class="text-body-2 text-grey">All fields are required.</p>
  <v-form @submit.prevent v-model="isMainFormValid" ref="mainForm">
    <div v-for="field in fields" :key="field.name">
            ...
            <v-card
              ...
              :style="[
                isHovering && isStudyTypeValid
                  ? { 'border-color': '#424242' }
                  : { 'border-color': '#9E9E9E' },
                !isStudyTypeValid ? { 'border-color': '#b00020' } : {}
              ]"
            >
              <v-radio-group
      ...
      <v-text-field
        ...
        :rules="
          field.name === 'Reporter' && vModel['case-study-type'] !== 'incident'
            ? []
            : [requiredRule]
        "
    ...
    <p class="text-h6 mb-0 mt-6">Procedure</p>
    <p class="text-body-2 text-grey">
      Construct a timeline of the incident, mapped to MITRE ATLAS&trade; techniques.
      <span :style="vModel.procedure.length === 0 && formSubmitted ? 'color: #FF5252' : ''"
        >Add at least one step.
      </span>
    </p>

    <EditableProcedureTimeline v-if="vModel.procedure.length > 0" v-model="vModel.procedure" />
    ...
    <div
      :style="{
        visibility:
          vModel.procedure.length === 0 && formSubmitted && !showAddNewStep ? 'visible' : 'hidden',
        color: '#c13951'
      }"
    ...
    <v-btn v-if="showAddNewStep" prepend-icon="mdi-plus" @click="showAddNewStep = false">
      Add New Step
    </v-btn>
    ...
    <v-row>
      ...
      <v-checkbox label="Also download as PowerPoint" v-model="downloadPptCheckbox" class="ml-6" />
    </v-row>

    <v-row class="align-center">
      <v-btn type="submit" color="blue" prepend-icon="mdi-download" @click="downloadCaseStudy">
        Download Case Study
      </v-btn>
      <v-alert
