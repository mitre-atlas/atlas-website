  ...
  <v-defaults-provider :defaults="formFieldDefaults">
    <div class="contribution-form-page ma-3 ma-md-0">
      <v-container class="pt-sm-16 mt-sm-16">
        ...
        <p>
          Thank you for your interest in contributing to MITRE ATLAS™! We highly value the
          collaboration of the AI community and want to learn more about your experiences and
          insights.
        </p>
        <p>
          Once a contribution is submitted, the MITRE ATLAS™ team will conduct a review and reach
          out with any follow-on actions. For any questions or issues, please contact
      ...
      <v-container>
        <v-tabs
          ...
          class="add-edit-tabs"
          @update:model-value="
            requestContributionSwitch({ action: $event, type: null, editTarget: null })
          "
        >
        ...
        <v-tabs-window :model-value="action">
          <v-tabs-window-item value="add">
            <div class="mt-6">
              <p>
                Select a contribution type from the options below to submit a new tactic, technique,
                mitigation, and/or case study for potential incorporation into the ATLAS Matrix. You
                may also select the “General Contribution” option to send us general thoughts,
                comments, or information for our awareness.
              </p>
              <v-select
                :model-value="type"
                :items="types"
              ...
              <div v-if="type && type !== 'other'">
                <p class="mt-12">
                  Already have an ATLAS {{ typeWordLower }} file? To view or edit your existing
                  {{ typeWordLower }}, upload your .yaml file using the “Upload File” button.
                </p>
                <p>
                  <b>Please note, uploading a file will overwrite all current field entries.</b>
                </p>
                <VAtlasBtnSecondary class="mt-2 mb-8" @click="showUploadDialog = true"
                  >Upload File</VAtlasBtnSecondary
                >
              </div>
            </div>
          </v-tabs-window-item>
          <v-tabs-window-item value="edit" class="pt-6">
            <p>
              Using the form below, indicate which ATLAS Matrix component you would like to submit
              for editing and the edits you recommend. Please note, edits will not automatically
              update the ATLAS Matrix upon submission. The MITRE ATLAS™ team will review the edit
              suggestion and reach out with any follow-on actions if the suggestion is approved.
            </p>
            <v-select
              :model-value="type"
              :items="editTypes"
        ...
        <UploadContributionDialog
          v-if="type && type !== 'other'"
          v-model="showUploadDialog"
          :type-key="type"
          @submit="handleContributionUpload"
        />
        <v-dialog v-model="showContributionSwitchDialog" max-width="520" persistent>
      ...
      <v-container :class="{ 'mt-8': type !== 'other', 'pt-0': type === 'other' }">
        ...
        <v-row v-if="showSubComponentEditForm" align="stretch" class="ma-0">
          ...
          <v-col cols="12" :sm="type === 'other' ? 12 : 9" class="pb-14">
            <form-structure
              ref="formStructureRef"
              :key="type"
              :type="type"
              :action="action"
              :sections="sectionsForForm"
              :edit-target="selectedEditTarget"
            />
          </v-col>
        </v-row>
...
const editTargets = computed(() => {
  ...
  return selectedType && selectedType !== 'other'
    ? mainStore
        .getDataObjectsByType(storeCollectionKey, matrixId)
...
const considerationRules = computed(() => {
  ...
  return [...typeRules, getResponsibleDisclosureConsideration(typeWordLower.value)]
})
...
function associatedTransformer(section, draft) {
  ...
  if (action.value === 'edit') {
    return {
      body: associatedArray
        .map(
          (element) =>
            `Add ${newAssociatedTransformer(element)} ${contributionTypeWordFromKey(associatedType)} Association`
        )
        .join('\n'),
    }
  }
  ...
  return {
    title: section.title + ':',
    body: associatedArray
      .map((element, idx) => `${idx + 1}. ${newAssociatedTransformer(element)}`)
      .join('\n'),
  }
}
...
function associationRemovalTransformer(section, draft) {
  ...
  return {
    body: removalArray
      .map(
        (element) =>
          `Remove ${newAssociatedTransformer(element)} ${contributionTypeWordFromKey(section.associatedType)} Association`
      )
      .join('\n'),
  }
}
...
function referenceTransformer(section, draft) {
  ...
  return {
    ...
    body: draft.references
      .map(
        (element, idx) =>
          `${idx + 1}. ${getReferenceDisplayText(element, REFERENCE_DISPLAY_MAX_CHARS)}`
      )
      .join('\n'),
  }
}
