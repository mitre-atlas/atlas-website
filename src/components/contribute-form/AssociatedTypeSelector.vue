<template>
  <v-autocomplete
    ...
    v-model:menu="menu"
    :label="requiredLabelIf(selectorLabel, props.required)"
    :required="props.required"
    :error="showRequiredError || hasExternalErrors"
    @update:focused="handleFocused"
    :items="existingDataObjects"
    chips
  ...
  <div v-if="props.allowNew && showNewItemForm" class="pl-6 mt-4">
    ...
    <v-text-field
      v-model.trim="newDraftItem.name"
      required
    ...
    <v-textarea
      v-model.trim="newDraftItem.summary"
      required
    ...
    ></v-textarea>
    <v-textarea
      v-if="showTechniqueUseField"
      v-model.trim="newDraftItem.use"
      required
...
const associatedHint = computed(() => {
  const subject = props.parentType
    ? `the ${contributionTypeWordFromKey(props.parentType, true)}`
    : 'this item'
...
function draftChipTitle(item) {
  const draftItem = isDraftItem(item?.raw)
    ? (getDraftItemFromModel(model.value) ?? item.raw)
    : item?.raw
  return draftItem?.name?.trim() || ''
}

function shouldSplitTechniqueTitle(item) {
  return props.type === 'techniques' && isSubtechnique(item.raw) && item.title.includes(': ')
}
...
function showNewItemFieldError(field) {
  return (
    getNewItemErrors(field).length > 0 &&
