<template>
  <div>
    <v-timeline :dense="mobile" side="end" class="text-right">
      <v-timeline-item
        max-width="1000"
      ...
      >
        ...
        <template v-if="!mobile" #opposite>
          <span :class="`text-h6 text-${getColor(event)} }`" v-text="event.date" />
          <div :class="`font-weight-light mb-4 text-${getColor(event)}`">
            {{ event.location }}
          </div>
        </template>
        <!-- Right of timeline -->
        <div class="py-4 text-left">
          <a :class="`text-h5 text-${getColor(event)}`" :href="event.url" target="_blank">
            {{ event.name }}
            <v-icon :color="getColor(event)" size="x-small"> mdi-open-in-new </v-icon>
          </a>
          <!-- Display date and location as subtitle if small screen -->
          <!-- Replaces "left of timeline" template contents -->
          <div v-if="mobile" :class="`font-weight-light mb-4 text-${getColor(event)}`">
            {{ event.date }} - {{ event.location }}
          </div>
          <div class="mt-3 tw-prose tw-max-w-none" v-html="md.render(event.description)" />
        </div>
      </v-timeline-item>
    </v-timeline>
  </div>
