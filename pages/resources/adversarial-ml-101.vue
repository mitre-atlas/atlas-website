<template>
  <div>
    ...
    <p>
      The methods underpinning the production machine learning systems are
    ...
    <p>Attacks on machine learning systems can be categorized as follows:</p>
    ...
    <p>
      ...
      attack is shown in Tay Poisoning Case Study where the attacker was able to
    ...
    <ol>
      <li>
        This does not cover all kinds of attacks -- adversarial ML is an active
        area of research with new classes of attacks constantly being
        discovered.
      </li>
      <li>
        Though the illustration shows blackbox attacks, these attacks have also
        been shown to work in whitebox (where the attacker has access to either
        model architecture, code or training data) settings.
      </li>
      <li>
        Though we were not specific about what kind of data – image, audio, time
        series, or tabular data - research has shown that of these attacks are
        data agnostic.
      </li>
    </ol>
    ...
    <p>
      Head over to the
      <NuxtLink :to="`/matrices/${getFirstMatrixId}`">
        {{ shortName }} Matrix
      </NuxtLink>
      page to see a compendium of attacks in MITRE ATT&CK<sup>&reg;</sup> style.
    </p>
  </div>
</template>
    ...
    attacksTableData: [
      {
        ...
        overview:
          'Attacker modifies a query in order to get a desired outcome. These attacks are performed by iteratively querying a model and observing the output.',
        type: 'Inference'
      },
      {
        attack: 'Functional Extraction',
        overview:
          'Attacker is able to recover a functionally equivalent model by iteratively querying the model. This allows an attacker to examine the offline copy of the model before further attacking the online model.',
        type: 'Inference'
      },
      {
        attack: 'Model Poisoning',
        overview:
          'Attacker contaminates the training data of an ML system in order to get a desired outcome at inference time. With influence over training data an attacker can create "backdoors" where an arbitrary input will result in a particular output. The model could be "reprogrammed" to perform a new undesired task. Further, access to training data would allow the attacker to create an offline model and create a Model Evasion. Access to training data could also result in the compromise of private data.',
        type: 'Train'
      },
      {
        attack: 'Model Inversion',
        overview:
          'Attacker recovers the features used to train the model. A successful attack would result in an attacker being able to launch a Membership inference attack. This attack could result in compromise of private data.',
        type: 'Inference'
      },
      {
