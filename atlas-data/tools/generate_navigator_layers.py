...
def normalize_atlas_data(data):
    """Normalize v5/v6 ATLAS YAML shapes for Navigator layer generation."""
    if "matrices" in data and "case-studies" in data:
        return data

    if "matrix" not in data or "tactics" not in data or "techniques" not in data:
        raise ValueError("Unsupported ATLAS YAML format for navigator layer generation")

    relationships = data.get("relationships", {})

    # Build ordered matrix tactics from sequence relationships when available.
    matrix_id = data["matrix"]["id"]
    sequence_rels = relationships.get(matrix_id, {}).get("sequences", [])
    ordered_tactic_ids = [
        rel["target"]
        for rel in sorted(sequence_rels, key=lambda rel: rel.get("position", 0))
    ]
    if not ordered_tactic_ids:
        ordered_tactic_ids = list(data["tactics"].keys())

    matrix_tactics = [
        data["tactics"][tactic_id]
        for tactic_id in ordered_tactic_ids
        if tactic_id in data["tactics"]
    ]

    # v6 techniques encode tactic membership in relationships[*].achieves.
    matrix_techniques = []
    for technique in data["techniques"].values():
        obj = deepcopy(technique)
        if obj["id"].count(".") == 1:
            achieves = relationships.get(obj["id"], {}).get("achieves", [])
            obj["tactics"] = [rel["target"] for rel in achieves if "target" in rel]
        matrix_techniques.append(obj)

    # v6 case study procedure is represented by relationships[*].employs.
    case_studies = []
    for case_study in data.get("case-studies", {}).values():
        obj = deepcopy(case_study)
        employs = relationships.get(obj["id"], {}).get("employs", [])
        procedure = sorted(employs, key=_step_sort_key)
        obj["procedure"] = [
            {"technique": step["target"], "tactic": step["tactic"]}
            for step in procedure
            if "target" in step and "tactic" in step
        ]
        obj.setdefault("summary", obj.get("description", ""))
        case_studies.append(obj)

    version = data.get("collection", {}).get("version") or data.get("version")

    return {
        "version": version,
        "matrices": [{"tactics": matrix_tactics, "techniques": matrix_techniques}],
        "case-studies": case_studies,
    }


def generate_individual_case_study_layers(
    matrix,
    output_dir,
    layer_data,
    ...
    for case_study in matrix["case-studies"]:
        ...
        for step in case_study["procedure"]:
            ...
            if is_subtechnique:
                ...
                technique_obj.update({"color": color})
                techniques.append(technique_obj)

                # Parse out the parent ID
                parent_technique_id_match = TECHNIQUE_ID_REGEX.match(technique_id)
                assert parent_technique_id_match is not None
                parent_technique_id = parent_technique_id_match.group()
                ...
                parent_technique_obj = deepcopy(
                    navigator_technique_objs[parent_technique_key]
                )
                ...
                technique_obj.update({"color": color, "showSubtechniques": False})
                techniques.append(technique_obj)
        ...
        individual_case_study_layer_data = {
            ...
            "legendItems": [{"label": "Used in case study", "color": color}],
            "metadata": [
                {
                    "name": "url",
    ...
    print(
        f"{len(matrix['case-studies'])} case study layer files outputted to {dir_path}"
    )


def generate_case_study_frequency_layer(
    matrix,
    output_dir,
    layer_data,
    navigator_technique_objs,
    layer_output_directory="default-navigator-layers",
):
    ...
    print(
        f"Case study frequency layer outputted to {dir_path / case_study_frequency_filename}"
    )
...
def generate_matrix_layer(
    ...
    layer_data,
    navigator_technique_objs,
    matrix_layer_directory="default-navigator-layers",
):
    ...
    matrix_layer_data = {
        ...
        "description": description,
        "techniques": techniques,
        "legendItems": [{"label": "ATLAS technique", "color": color}],
    }
    # Combine with default layer data
...
def build_navigator_technique_objs(data):
    ...
    for matrix in data["matrices"]:
        for tactic in matrix["tactics"]:
            ...
            navigator_tactic_names[tactic["id"]] = (
                tactic["name"].replace(" ", "-").lower()
            )
...
def main() -> None:
    ...
    if args.layer is None or args.layer == "case_study":
        ...
        generate_case_study_frequency_layer(
            data, output_dir, layer_data, navigator_technique_objs
        )
        # Generate highlight layers for techniques used in each case study
        generate_individual_case_study_layers(
            data, output_dir, layer_data, navigator_technique_objs
        )
